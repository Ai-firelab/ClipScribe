...
async function downloadReel({ url, outputDir, maxRetries = 3, proxyUrl, instagramCookiesJson, timeoutMs = 5 * 60 * 1000 }) {
    ...
    if (instagramCookiesJson) {
        try {
            ...
            // Expect an array of { name, value, ... } objects (Netscape format adapted).
            // Write as simplified Netscape cookies format that yt-dlp understands.
            const lines = [
                '# Netscape HTTP Cookie File',
                '# This file is generated by the ClipScribe actor.',
            ];
            for (const cookie of cookies) {
                const { name, value, domain = '.instagram.com', path: cookiePath = '/', secure = true } = cookie;
                if (!name || !value) continue;
                const parts = [domain, 'TRUE', cookiePath, secure ? 'TRUE' : 'FALSE', '0', name, value];
                lines.push(parts.join('\t'));
            }
            cookiesFile = path.join(outputDir, 'cookies.txt');
            await fs.writeFile(cookiesFile, lines.join('\n'), 'utf8');
            log.info('Using Instagram cookies for authenticated metadata extraction.');
        } catch (err) {
